...
Fixpoint run_cprop {C:Ctx} (cprop : CProp C)
  ...
  | Implies _ prop cprop' =>
      fun cenv fenv => 
        ...
        | true => run_cprop cprop' cenv fenv   
        | false => None
        end
  | Check _ prop =>
...
Definition size_log2 (passed discards : nat) : nat :=
  Nat.log2 (passed + discards)%nat.

Definition size_const5 (_passed _discards : nat) : nat := 5.

Definition size_linear (passed discards : nat) : nat :=
  let s := S ((passed + discards) / 64) in
  if Nat.leb s 64 then s else 64.

Definition size_cyclic (passed discards : nat) : nat :=
  match (passed + discards) mod 4 with
  | 0 => 2
  | 1 => 5
  | 2 => 10
  | _ => 20
...
Definition runLoopWithSize
  ...
  let fix runLoop'
    ...
    | S fuel' =>
        res <- generate_and_run cprop (size_fn passed discards);;
        match res with
        | Normal seed false =>
            (* Fails *)
...
Definition runLoop (fuel : nat) (cprop : CProp ∅): G Result :=
  runLoopWithSize size_log2 fuel cprop.
...
#[export] Instance Untuple_empty : Untuple nat :=
  { untuple := ∅
  ; untuple_correct := eq_refl }.

Require Import Program.Tactics.

#[export] Program Instance Untuple_pair {A B} `{Untuple B} : Untuple (A * B) :=
