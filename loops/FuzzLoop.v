...
Definition fuzzLoop
	...
	let fix fuzzLoop' 
      ...
      | S fuel' => 
          ...
          input <- generator cprop directive (log2 (passed + discards));;
					...
					| None =>
						...
						fuzzLoop' fuel' passed (discards + 1) seeds' utility
					| Some input =>
						...
						| Some true =>
								...
								| true =>
										...
										fuzzLoop' fuel' (passed + 1) discards seeds' utility
								| false =>
										let seeds' := match directive with
																	| Generate => seeds
																	| Mutate _ => revise seeds
						...
						| None => 
								...
								match useful seeds feedback with
								| true =>
									(* TODO: We currently don't leverage discarded useful inputs *)
									fuzzLoop' fuel' passed (discards + 1) seeds utility
								| false =>
									let seeds' := match directive with
																| Generate => seeds
      ...
      fuzzLoop' fuel 0 0 seeds utility.